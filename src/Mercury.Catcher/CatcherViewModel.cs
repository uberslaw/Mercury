            ...
            _server = new CatcherHttpsServer(
                ...
                info => _dispatcher.BeginInvoke(() =>
                {
                    LastTransfer = info.Ok
                        ? $"{TransferRundown.FormatLocal(info.Utc)}  {info.FileName}  {ByteFormatter.ToString(info.Bytes)}  {info.Message}"
                        : $"{TransferRundown.FormatLocal(info.Utc)}  FAILED  {info.FileName}  {info.Message}";
                }));
            _server.Start();
            IsListening = true;
